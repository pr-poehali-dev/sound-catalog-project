...
export default function HomePage({ onNavigate, onPlay, currentTrackId, isPlaying }: HomePageProps) {
  ...
  const heroPlaying = isPlaying && currentTrackId !== null;
        ...
        <div className="space-y-2">
          {filtered.map((track, idx) => {
            ...
            return (
              <div
                key={track.id}
                className={`track-card glass rounded-xl px-5 py-4 flex items-center gap-4 group cursor-pointer border ${
                  active ? 'border-primary/30 bg-primary/5' : 'border-transparent'
                }`}
              >
                {/* Number / Play */}
                <div className="w-8 text-center flex-shrink-0">
                  <span className={`font-unbounded text-xs ${active ? 'hidden' : 'text-foreground/20 group-hover:hidden'}`}>
                    {String(idx + 1).padStart(2, '0')}
                  </span>
                  <button
                    onClick={() => onPlay(track)}
                    className={`w-8 h-8 rounded-full bg-primary/10 border border-primary/30 items-center justify-center hover:bg-primary/20 transition-colors ${
                      active ? 'flex' : 'hidden group-hover:flex'
                    }`}
                  >
                    <Icon name={playing ? 'Pause' : 'Play'} size={14} className="text-primary ml-0.5" />
                  </button>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className={`font-unbounded font-semibold text-sm truncate ${active ? 'text-primary' : ''}`}>{track.title}</div>
